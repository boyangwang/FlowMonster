var App = {
    _isWithTooltips: !1,
    init: function() {
        App._tooltips(), App._navDoc(), $(window).on("resize", App._tooltips), $(document).on("shown.bs.tab", function() {
    ...
    _tooltips: function() {
        if ($(window).width() > 768) {
            ...
            App._isWithTooltips = !0, $('[data-hover="tooltip"]').tooltip()
        } else {
            if (!App._isWithTooltips) return;
            App._isWithTooltips = !1, $('[data-hover="tooltip"]').tooltip("destroy")
        }
    },
